...
function SignIn() {
  ...
  const mutation = useMutation({
    mutationKey: ["signIn"],
    mutationFn: () =>
      axios.post('http://localhost:5000/api/v1/user/sign-in', {
        username,
        password
      }),
    onSuccess: ({data}) => {
    ...
    onError: (error: AxiosError<{msg: string}>) => {
      const message = error?.response?.data?.msg || "Sign in failed"
      toast.error(message)
    }
  })
  return (
    <>
        <div className="bg-gray-100 w-screen h-screen flex justify-center items-center">
            <div className="bg-white max-w-137.5 w-[27%] border border-border shadow-lg px-12 py-10 rounded-xl">
              <h1 className={cn(textVariants({variant: "h2"}), "text-gray-600 text-center pb-5")}>Log in</h1>
              ...
              <div>
                <Button disabled={mutation.isPending} onClick={() => mutation.mutate()} className={`w-full py-2.5 ${mutation.isPending? "cursor-not-allowed bg-[#7f7af0]": "cursor-pointer"}`}>{mutation.isPending? "Signing In": "Sign In"}</Button>
              </div>
              <div onClick={() => navigate('/sign-up')} className="pt-3 text-right text-blue-600 cursor-pointer underline">
                Don't have account?
              </div>
            </div>
        </div>
